...
class BakeryEnvironmentScene extends Phaser.Scene {
    ...
    checkInteractions() {
        ...
        if (nearest) {
            ...
            if (Phaser.Input.Keyboard.JustDown(this.interactKey)) this.executeInteraction(nearest);
        } else {
    ...
    update(time) {
        ...
        const speed = 180;
        let vx = 0, vy = 0;
        if (this.cursors.left.isDown || this.wasd.left.isDown) { vx = -speed; this.player.flipX = true; }
        else if (this.cursors.right.isDown || this.wasd.right.isDown) { vx = speed; this.player.flipX = false; }
