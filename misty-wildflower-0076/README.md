...
This project now includes a direct Workers AI endpoint for websites:
...
```json
{
  "prompt": "Write a short tip for saving money this month",
  "system": "You are a concise financial coach",
...
### CORS and model config

Configure these in `wrangler.jsonc`:

- `ALLOWED_ORIGIN`: set to your frontend origin (for example `https://your-site.com`) in production.
- `AI_MODEL`: default Workers AI model used when `model` is not sent in the request.

### Frontend example

```ts
const res = await fetch("https://<your-worker>.workers.dev/api/ai", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ prompt: "How can I reduce grocery spending?" })
});
...
### GitHub Codespaces auth (no localhost callback)

If your browser blocks `localhost` OAuth redirects (common on managed school devices), use an API token instead of `wrangler login`.

1. In Cloudflare dashboard, create an API token with at least:
  - `Workers Scripts:Edit`
  - `Workers AI:Edit`
  - `Account:Read`
2. In your Codespace, export credentials:
...
3. Verify auth without browser login:
...
```bash
npm run dev
# open /ai-demo.html and ask a question
```

Note: `wrangler login --callback-port 3000` still redirects through localhost in current Wrangler behavior, so token auth is the most reliable Codespaces path.
