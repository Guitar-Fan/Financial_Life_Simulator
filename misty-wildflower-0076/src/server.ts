...
function getCorsHeaders(request: Request, env: RuntimeEnv) {
...
async function handleWebsiteAIRequest(request: Request, env: RuntimeEnv) {
  ...
  try {
    ...
    const model = body.model?.trim() || env.AI_MODEL || DEFAULT_MODEL;

    const result = await generateText({
      model: workersai(model),
      system: body.system,
      prompt,
      maxOutputTokens: Math.min(Math.max(body.maxOutputTokens ?? 512, 64), 2048)
    });
    ...
    return new Response(
      JSON.stringify({
        error: "Failed to run AI request.",
...
export default {
  async fetch(request: Request, env: RuntimeEnv) {
    ...
    if (url.pathname === "/api/ai") {
      return handleWebsiteAIRequest(request, env);
    }
